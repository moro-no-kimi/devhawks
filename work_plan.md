...
Baseline GitHub ruleset `23971563` is active on the default branch: no bypass actors, deletion/force-push prevention, required PR approval with stale-review dismissal and latest-push approval, resolved review threads, and automatic Copilot review on new pushes. Effective branch rules were read back successfully. This is not yet G1 acceptance: required CI/trusted current-head gate enforcement and its negative tests still need demonstration.
